...
int main()
{
    ...
    auto lcdLoc         = cPgr.ReadPoint("LcdXY");              // Get the location of various things
    auto dialLoc        = cPgr.ReadPoint("DialXY");             // which are pretty obvious by their titles.
    auto debugLoc       = cPgr.ReadPoint("DebugWinXY");
    auto buttonLoc      = cPgr.ReadPoint("ButtonXY");
    auto debugSize      = cPgr.ReadSize("DebugWinSize");
    auto debugColor     = cPgr.ReadText("DebugWinColor"); 
    auto bitmap         = cPgr.ReadBitmap("texture");
    ...
    auto& win = Sagebox::NewWindow(100,100,bitmap.GetWidth(),bitmap.GetHeight(),"Sagebox -- Using two Widgets / Simple Emulation Example",NoBorder());

    win.DisplayBitmap(bitmap);        // Display the background image.
    ...
    CLcdDisplayWidget lcd(&win,lcdLoc,0,Transparent()); 
    ...
    auto& cDebugWin = win.ChildWindow(debugLoc,debugSize,bgColor(debugColor) | Font("Courier New,12")); 
    ...
    auto InitDial = [&]()
    {
        dial.SetRange(0,150);
        dial.SetValue(75);
        lcd.SetValue((int) dial.GetValue());
    };
    ...
    {
        double value;
        if (dial.event.ValueChanged(value))
        {
            cDebugWin.printf("{g}Event.{/}  Value = {y}%d{/} -- Raw Value = {c}%d{/} -- Sent Value to Device (LCD)\n",(int) value,dial.GetRawValue());
            lcd.SetValue((int) value);
        }
    };
    ...
    auto &cButtonClose = win.NewButton(buttonLoc.x,buttonLoc.y,120,0,"Exit");
    auto &cButtonAbout = win.NewButton(buttonLoc.x+125,buttonLoc.y,120,0,"About");
    ...
    while(win.GetEvent() && !cButtonClose.Pressed())
    {
        ...
        if (cButtonAbout.Pressed()) win.DrawSimpleDoc(sAbout, Modal() | CenterXY());   // Options: Modal()     -- this keeps the other windows from being used until the
