...
// ------------------------
// 
// 1. Simple Draw Vector Call                   --  Calls CWindow::DrawVector().
// 2. Using CDrawVector Class (this example)    --  Uses the CDrawVector class directoy for a more refined and flexible way to draw and change vectors.
...
// 
// --> Using CDrawVector Class.  This example creates a CDrawVector object with some initial setup, and then re-uses the same vector over and over.
//                               This can be a more efficient use of vectors in real-time situations or otherwise where the same vector can be reused.
// 
//                               It also allows some pre-setup of a vector with only small changes, avoiding the need to setup a new vector each time when
//                               using CWindow::DrawVector().
// 
...
// 
//         5. AngleRad(fDispAngle+Math::Rad90)   -- Since SetCenter() is used, and the angle is 0 between P1 and P2, this rotates the vector from it's 
...
int main()
{
    ...
    auto pWinCenter = win.GetWindowCenter() + CPoint{0,60};    // Get the window center + a little vertical offset
    ...
    // Create the inistial CDrawVector object.
    //
    // We can use the keywords in the constructor to set up some initial, persitent values. 
    // Later, we'll change the vector as we need to to display it how we want.

    CDrawVector vector(win,{0,0},{350,0},10,vkw::LabelJust("leftcenter") + vkw::Color(PanColor::Orange));
    ...
    while(win.GetEvent() && !Sagebox::DevWindowClosed())
    {
        ...
        win.Write(0,70,"{50}Sagebox C++ DrawVector() Function Test\n"
                        "{18}{lightgreen}CDrawVector Class usage example",kw::JustCenterX());

        win.DrawCircle(pWinCenter,radius,SageColor::LightBlue,Sage::kw::PenSize(15));  // Draw the large circle in light blue with a 15-pixel radius
        win.FillCircle(pWinCenter,15,SageColor::LightBlue);                             // Draw the small circle as a filled circle.

        // Move around the circle, drawing a vector at each point

        auto fDispAngle = vecPos+vecAngle; 
        if (fDispAngle >= Math::Rad360) fDispAngle -= Math::Rad360f; 
        ...
        CPoint pOut = CPoint{(int) x,(int) y} + pWinCenter;      // Where to put the vector center.

        auto title = CString() << "Rotation Angle = " << (int) Math::ToDeg(vecPos) << "°";   // Title for the vector.

        // Draw the vector with various keywords, setting keywords for only the things we change on every display, since
        // we set up the color and other things before or main loop (when we created the vector)
        // 
        //  See notes above for how these keywords are used.
        //
        ...
        vector.Draw(vkw2::SetCenter = (CfPoint) pOut,vkw::Title(title) + vkw::AngleRad(fDispAngle+Math::Rad90));
